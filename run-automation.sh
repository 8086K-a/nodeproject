...
3. Test thoroughly (run npm run lint and npm run build in ICVN-Project-master/)
...
PROMPT_EOF
    ...
    # Using --skip-git-repo-check because this root folder is not a git repo
    # Using --dangerously-bypass-approvals-and-sandbox for unattended automation
    if codex exec \
        --skip-git-repo-check \
